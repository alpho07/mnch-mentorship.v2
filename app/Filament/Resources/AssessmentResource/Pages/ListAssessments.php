                    ...
                    Tables\Actions\Action::make('manage_team')
                        ...
                        ->form([
                            ...
                            \Filament\Forms\Components\Placeholder::make('other_potential_members_heading')
                                ->hiddenLabel()
                                ->content(new \Illuminate\Support\HtmlString('<hr class="border-gray-200 dark:border-gray-700 mb-4"><h3 class="text-base font-semibold text-gray-950 dark:text-white">Add Team Members</h3><p class="text-sm text-gray-500">Search for anyone by name or email — they don\'t need to already be an assessor.</p>'))
                                ->columnSpanFull(),
                            \Filament\Forms\Components\Select::make('member_ids')
                                ->label('Add team members')
                                ->multiple()
                                ->searchable()
                                ->getSearchResultsUsing(fn (string $search, Assessment $record): array => app(\App\Services\AssessmentTeamService::class)
                                    ->searchEligibleUsers($record, $search)
                                ...
                                ->getOptionLabelsUsing(fn (array $values): array => User::whereIn('id', $values)
                                    ...
                                    ->all())
                                ->columnSpanFull()
                                ->helperText('Anyone added who isn\'t already an assessor will be granted the Assessor role.'),
                        ])
