...
class AnalyticsDashboardController extends Controller
{
    ...
    private function getChartData($year, $mode, $trainingId = null, $countyId = null, $subcountyId = null, $facilityId = null)
    {
        ...
        for ($i = 5; $i >= 0; $i--) {
            ...
            $count = TrainingParticipant::whereHas('training', function ($query) use ($trainingType, $year, $trainingId, $mode) {
                $query->where('type', $trainingType);
                if ($mode !== 'training') {
                    $this->applyLiveMentorshipConstraint($query);
                }
    ...
    private function getSummaryStatsSimple($year, $mode, $trainingId = null, $countyId = null, $subcountyId = null, $facilityId = null)
    {
        ...
        try {
            ...
            $totalParticipantsQuery = TrainingParticipant::whereHas('training', function ($query) use ($trainingType, $year, $trainingId, $mode) {
                $query->where('type', $trainingType);
                if ($mode !== 'training') {
                    $this->applyLiveMentorshipConstraint($query);
                }
    ...
    private function getExtendedStats(string $year, string $mode, $countyId = null, $subcountyId = null, $facilityId = null): array
    {
        ...
        try {
            ...
            if (! empty($year)) {
                $statusQuery->whereYear('start_date', $year);
            }
            if ($hasGeoFilter) {
                if ($mode === 'training') {
            ...
            for ($i = 11; $i >= 0; $i--) {
                ...
                if ($mode === 'training') {
                    ...
                    $cnt = Training::where('type', 'facility_mentorship')->liveMentorship()
            ...
            $prevYear = $curYear - 1;
            if ($mode === 'training') {
                $curCount = TrainingParticipant::whereHas('training', fn ($q) => $q->where('type', $trainingType)->whereYear('start_date', $curYear))
                    ->when($hasGeoFilter, fn ($q) => $q->whereHas('user.facility', $geoScope))
                    ->count();
                $prevCount = TrainingParticipant::whereHas('training', fn ($q) => $q->where('type', $trainingType)->whereYear('start_date', $prevYear))
                    ->when($hasGeoFilter, fn ($q) => $q->whereHas('user.facility', $geoScope))
                    ->count();
            } else {
                $pilotFilter = fn ($q) => $q->liveMentorship();
            ...
            $totalParticipants = $mode === 'training'
                ...
                : ClassParticipant::whereHas('mentorshipClass.training', function ($q) use ($year, $hasGeoFilter, $geoScope) {
                    $q->where('type', 'facility_mentorship')
                        ...
                        ->whereHas('mentorshipClasses.participants', fn ($sq) => $sq->whereIn('status', ['enrolled', 'active', 'completed']));
                    if (! empty($year)) {
                        $q->whereYear('start_date', $year);
                    }
                    if ($hasGeoFilter) {
            ...
            $topPrograms = $mode === 'training'
                ? Training::where('type', 'global_training')
                    ...
                    ->toArray()
                : Training::where('type', 'facility_mentorship')->liveMentorship()
                    ->when(! empty($year), fn ($q) => $q->whereYear('start_date', $year))
                    ->when($hasGeoFilter, fn ($q) => $q->whereHas('facility', $geoScope))
                    ->get(['id', 'title', 'status', 'start_date'])
                    ->map(function ($t) {
    ...
    private function generateInsights(array $summaryStats, array $extendedStats, string $mode): array
    {
        ...
        // Coverage insight
        if ($coverage >= 70) {
            $insights[] = ['type' => 'success', 'icon' => 'check-circle', 'text' => "Strong facility coverage at {$coverage}% — {$label} programs are reaching a broad base of healthcare facilities."];
        } elseif ($coverage >= 35) {
            $insights[] = ['type' => 'warning', 'icon' => 'exclamation-triangle', 'text' => "Moderate facility coverage at {$coverage}% — ".number_format($summaryStats['totalFacilities'] ?? 0).' facilities active; consider expanding to uncovered areas.'];
        } elseif ($programs > 0) {
            $insights[] = ['type' => 'danger', 'icon' => 'exclamation-circle', 'text' => "Low facility coverage at {$coverage}% — significant expansion of {$label} programs needed to reach more healthcare facilities."];
        }

        // Participant throughput
        if ($programs > 0 && $avg > 0) {
            $insights[] = ['type' => 'info', 'icon' => 'users', 'text' => "Average of {$avg} {$pLabel} per {$label} across {$programs} programs — total reach: ".number_format($participants)." unique {$pLabel}."];
        }
        ...
        if (! empty($yoy) && ($yoy['current'] > 0 || $yoy['previous'] > 0)) {
            ...
            $year = $yoy['year'];
            if ($change > 5) {
                $insights[] = ['type' => 'success', 'icon' => 'arrow-up', 'text' => "{$change}% year-on-year increase in {$year} — {$label} program activity is growing strongly."];
            } elseif ($change < -5) {
                $abs = abs($change);
                $insights[] = ['type' => 'warning', 'icon' => 'arrow-down', 'text' => "{$abs}% decline in {$year} versus prior year — review {$label} program enrollment and engagement strategies."];
            } else {
                $insights[] = ['type' => 'info', 'icon' => 'minus-circle', 'text' => "Stable year-on-year activity ({$change}% change) — {$year} {$label} performance is consistent with prior year."];
            }
        }
        ...
        if (! empty($topCounties)) {
            $top = $topCounties[0];
            $insights[] = ['type' => 'primary', 'icon' => 'map-marker-alt', 'text' => "{$top['name']} county leads with ".number_format($top['count'])." {$pLabel} — a high-impact county for {$label} scale-up."];
        }
        ...
        if ($total > 0 && ($active > 0 || $completed > 0)) {
            ...
            $insights[] = ['type' => 'info', 'icon' => 'chart-pie', 'text' => "{$activeRate}% of {$label} programs are active; {$completedRate}% completed".
        ...
        if ($mode === 'mentorship') {
            ...
            if ($attendanceRate > 0) {
                if ($attendanceRate >= 80) {
                    $insights[] = ['type' => 'success', 'icon' => 'clipboard-check', 'text' => "Strong module attendance at {$attendanceRate}% — mentees are consistently showing up for sessions."];
                } elseif ($attendanceRate >= 60) {
                    $insights[] = ['type' => 'warning', 'icon' => 'clipboard-check', 'text' => "Module attendance is {$attendanceRate}% — consider follow-up with mentors to improve session attendance."];
                } else {
                    $insights[] = ['type' => 'danger', 'icon' => 'clipboard-check', 'text' => "Low module attendance at {$attendanceRate}% — review scheduling, mentor engagement, and mentee barriers."];
                }
            }
            ...
            if ($totalMenteesWithProgress > 0) {
                ...
                if ($completionRate >= 70) {
                    $insights[] = ['type' => 'success', 'icon' => 'graduation-cap', 'text' => "{$completionRate}% of mentees have completed all assigned modules — mentorship completion is on track."];
                } elseif ($completionRate >= 40) {
                    $insights[] = ['type' => 'warning', 'icon' => 'graduation-cap', 'text' => "{$completionRate}% of mentees have completed all modules — monitor those in lower completion buckets."];
                } else {
                    $insights[] = ['type' => 'danger', 'icon' => 'graduation-cap', 'text' => "Only {$completionRate}% of mentees have completed all modules — significant support intervention may be needed."];
                }
            }
            ...
            if ($activeClasses > 0 || $draftClasses > 0) {
                $insights[] = ['type' => 'primary', 'icon' => 'chalkboard-teacher', 'text' => "Class pipeline: {$activeClasses} active classes and {$draftClasses} draft classes across mentorship programs."];
            }
        }
