...
### 9.16 `GET /api/v1/sections` and `/api/v1/sections/{section}` were completely broken — **FIXED 2026-08-06**
`App\Http\Resources\Api\AssessmentSectionResource`, used by both actions in `AssessmentSectionController`, didn't exist anywhere in the codebase — a fatal `Class not found` error on every single request, mobile app included (found via `ApiRouteSmokeTest`). Unlike most findings this session, there was zero ambiguity about the correct fix: the sibling `fullSchema()` method in the same controller, plus the two existing sibling classes already in `app/Http/Resources/Api/` (`AssessmentResource`, `UserResource`), together specified exactly what the missing class needed to contain. Created it, verified both `index` and `show` return correct data end-to-end against real model data, and added `tests/Feature/Api/AssessmentSectionApiTest.php` as permanent regression coverage beyond the smoke test.

### 9.17 Monthly Reports Create form was 100% unusable — `IndicatorValue` never matched the real `indicator_values` schema — **FIXED 2026-08-06**
Restoring `monthly_reports` (§9.1a) and seeding a real `ReportTemplate` (this section's own fix, see below) still wasn't enough to make the feature usable — `CreateMonthlyReport`'s `indicatorValues` Repeater crashed on page **load**, before any submission, because it queries `indicator_values` by `monthly_report_id` (`Illuminate\View\ViewException: no such column: indicator_values.monthly_report_id`). Investigating further: `IndicatorValue`'s entire `$fillable` (`monthly_report_id`, `indicator_id`, `numerator`, `denominator`, `calculated_value`, `comments`) matched the real table on `indicator_id` alone — the live `indicator_values` table (43 real rows) has always used a completely different shape (`period_id`, `numerator_value`, `denominator_value`, `computed_percentage`, `comment`) belonging to the separate, already-working DHIS2-style `indicator_report_periods` reporting flow. The Monthly Reports code was written against a schema that was apparently never actually built — consistent with `monthly_reports`' own migration having been deleted twice in git history (§9.1a).

**Fix (user-approved after being presented with the full scope, not assumed):** added a migration for the 4 missing nullable columns (`monthly_report_id` FK cascade-delete, `numerator`, `denominator`, `calculated_value`) plus `comments`, and relaxed the existing `period_id` column from `NOT NULL` to nullable (Monthly-Report-sourced rows never set it; the existing 43 period-based rows are completely unaffected — pure relaxation, not a data change). The two "shapes" now coexist in the same table: period-based rows leave the 4 new columns NULL, Monthly-Report-based rows leave `period_id` NULL. MySQL's unique index on `(period_id, indicator_id)` allows multiple NULLs, so this doesn't collide. Verified with a full end-to-end test (`MonthlyReportCreateSubmissionTest`) that actually submits the Filament form and confirms both the `MonthlyReport` and its `IndicatorValue` rows are created correctly.

**Also fixed in this pass:** seeded one real `ReportTemplate` (`MONTHLY_FACILITY_INDICATORS`, `report_type: general`, all real indicators attached) via a new standalone `ReportTemplateSeeder`, and added defensive helper text on the `report_template_id` Select field for if the dropdown is ever empty again. See `docs/superpowers/specs/2026-08-06-monthly-reports-empty-state-design.md` and its implementation plan for the full design.
