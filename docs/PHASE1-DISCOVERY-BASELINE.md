...
### 9.12 Three admin dashboards use MySQL-only SQL functions, untestable against SQLite — **Medium** (found 2026-08-06)
`admin/coverage-dashboard`, `admin/coverage-overview`, and `admin/training-dashboard` all run raw queries using `DATE_FORMAT()`, `YEAR()`, and/or `MONTH()` directly (e.g. `app/Filament/Pages/TrainingCoverageDashboard`-family code, and `resources/views/filament/pages/training-coverage-dashboard.blade.php`). These functions don't exist in SQLite, which `phpunit.xml` uses for the entire test suite — confirmed working fine against the real MySQL database (queries execute without error there). This isn't an app bug, but it is a real, structural **testing gap**: these three dashboards (and likely others using the same query style, not exhaustively searched) can never be covered by this test suite as currently configured, only by manual/browser verification or by switching CI to a MySQL service container instead of SQLite. Excluded from the new route smoke test (`tests/Feature/AdminRouteSmokeTest.php`) with this reasoning inline; worth a deliberate decision later — rewrite the queries to be portable (e.g. Carbon-based grouping in PHP, or `strftime()` for SQLite / `DATE_FORMAT()` for MySQL via a query-builder helper) vs. accept MySQL-only CI.
